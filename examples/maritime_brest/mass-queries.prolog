...
update_global_vars(lles):-
	...
	findall(T, (event(F), happensAt(F, [T])), Hap), length(Hap, Happens),
	findall(X, (inputEntity(F=V), F \= proximity(_,_), holdsFor(F=V,L), length(L,X)), HF), sum_list(HF, HoldsFor),
	findall(X, (inputEntity(F=V), F = proximity(_,_), holdsFor(F=V,L), L \== [], length(L,X)), SHF),  sum_list(SHF, SpHoldsFor),

	findall(T, (event2(F), (F = entersArea(_) ; F = leavesArea(_)), happensAt(F, T)), SpHap),
	length(SpHap, SpHappens),
...
update_global_vars(overlapping, NextInit) :-
	...
	findall(T, (event(F), happensAt(F, [T]), T > NextInit), Hap), length(Hap, Happens),
